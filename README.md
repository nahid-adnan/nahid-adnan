# Hi, I'm Nahid 👋
**Data science postgrad at JCU (Cairns) — building toward a data analyst career in Australia.**
I came into data science from statistics (MSc, Jahangirnagar University) and I'm now completing my 
Master of Data Science at James Cook University. This profile is where I keep the work I'm learning 
from and building — Python, R, Power BI, SQL, and the occasional experiment.
---
## 🧰 Tools I work with
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=for-the-badge&logo=r&logoColor=white)
![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![Excel](https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white)
![Power Query](https://img.shields.io/badge/Power%20Query-F2C811?style=for-the-badge&logo=powerbi&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
---
## 🚀 What I'm doing now
- Studying data visualisation and data ethics as part of my MDS at JCU
- Building Python and analysis projects to sharpen the fundamentals
- Working toward a data analyst role in Brisbane on a 485 visa
---
## 🔨 A few things I've built
- [**Tasmania-short-stay-dashboard**](https://github.com/nahid-adnan/Tasmania-short-stay-dashboard) — An 
interactive Power BI dashboard on Tasmania's short-term rental market: 5,293 listings across five report 
pages, with a bubble map of estimated revenue by area, a repricing what-if scenario, host portfolio 
analysis, and a drill-through listing register. Built with DAX measures, what-if parameters, drill-through, 
and a custom tooltip page.
- [**blue-v-red**](https://github.com/nahid-adnan/blue-v-red) — A turn-based command-line Python 
game where name length determines win probability, with a pandas/matplotlib data analysis component 
built in. Written under strict constraints: no global variables, no `while True`.
- [**cp5639-python-practicals**](https://github.com/nahid-adnan/cp5639-python-practicals) — A 
portfolio of Python practicals covering conditionals, loops, functions, and lists. Built as part of 
my MDS programming coursework at JCU.
- [**cp1401-assignment-1**](https://github.com/nahid-adnan/cp1401-assignment-1) — Python work 
covering IPO design, conditionals, loops, and input validation.
- **Power BI Dashboards** *(coming soon)* — Visualisation projects built during my MDS, including 
an urban mobility dashboard and a sustainable activewear sales analysis for North Queensland.
---
## 📊 Activity
- 🐍 Most of my current work is in Python
- 📈 Building toward adding R and Power BI projects publicly
- 🔄 10 contributions in the last year — actively growing this profile
---
## 📬 Reach me
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/nahid-hasan-a8767114b)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:nahid316unmad@gmail.com)
---
*Based in Cairns, Queensland 🇦🇺 | Open to data analyst opportunities in Brisbane*
